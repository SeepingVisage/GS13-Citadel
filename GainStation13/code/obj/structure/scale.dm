...
/mob/living/carbon/proc/calculate_weight_in_pounds()
	var/returnweight = 140
	var/pounds = fatness*FATNESS_TO_WEIGHT_RATIO
	var/taur = (dna.features["taur"] != "None") ? 2.5 : 1
	var/voreweight = 0
	for(var/belly in vore_organs)
		for(var/prey in belly)
			if(istype(prey, /mob/living/carbon))
				var/mob/living/carbon/B = prey
				voreweight += B.calculate_weight_in_pounds()
	returnweight += pounds
	returnweight *= size_multiplier**2
	returnweight *= taur
	returnweight += voreweight
	return returnweight
